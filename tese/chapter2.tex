...
was popularity. At the same time, we tried to pick languages from different
...
(such as WebPPL, Church and Figaro) is that we can display results to the user
...
WebPPL was written as part of a course on PP and inference in PPLs \cite{dippls}.
...
to be the target of our VPE is that, because it was written in javascript, it runs
in the browser. The advantage behind that is that is makes the language compatible
...
it also has an implementation in javascript called webchurch \cite{chater2006probabilistic},
...
in our VPE, the natural choice would be VentureScript.
...
besides the algorithm used, is that WinBUGS provides a bi-direcitonal conversion
